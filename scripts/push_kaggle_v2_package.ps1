# Publish the V2 bundle/cache datasets, then push the V2 package CPU kernel.
...
$kernelDir = Join-Path $kaggleDir "kernel-p"
...
Publish-Dataset -Folder (Join-Path $kaggleDir "dataset-bundle") `
    -Message "V2 package bundle refresh"
...
Write-Output ("Monitor with: kaggle kernels status $kernelSlug ; " +
    "fetch results with scripts\fetch_kaggle_v2_package_results.ps1")
