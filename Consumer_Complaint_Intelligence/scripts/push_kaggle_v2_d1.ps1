# Publish the V2-D1 bundle and cache datasets, then push the Kaggle kernel.
...
$metaFiles = @(
    (Join-Path $kaggleDir "kernel-metadata.json"),
...
& $KaggleExe kernels push -p $kaggleDir
if ($LASTEXITCODE -ne 0) {
    throw "Kernel push failed"
}

# Read the kernel slug from the metadata so the two cannot drift apart.
$kernelMeta = Get-Content (Join-Path $kaggleDir "kernel-metadata.json") -Raw |
    ConvertFrom-Json
...
Write-Output ("Monitor with: kaggle kernels status $kernelSlug ; " +
    "fetch results with scripts\fetch_kaggle_v2_d1_results.ps1")
